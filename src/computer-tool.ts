...
import type { ComputerTool, ComputerToolOptions, ToolResult } from "./types.js";
...
export function createComputerTool(options?: ComputerToolOptions): ComputerTool {
  if (options?.lockDir) {
    configureLockDir(options.lockDir);
  }

  if (options?.debugArtifacts !== undefined) {
    configureDebugArtifacts({
      enabled: options.debugArtifacts,
      dir: options.debugArtifactsDir,
    });
  }
  ...
  async function execute(
    ...
    setActiveDebugArtifactRunId(toolCallId);
        ...
        switch (action) {
          case "screenshot":
            return await executeScreenshot({
              displayIndex: args.display_index as number | undefined,
          ...
          case "click":
            return (
              evaluateActionGuardrail({ action, x: args.x as number, y: args.y as number }) ??
              (await executeClick({
                x: args.x as number,
                y: args.y as number,
          ...
          case "type":
            return await executeType({ text: args.text as string, signal }).then((result) => {
              recordRecentTextEntry({
                action: "type",
          ...
          case "press":
            return await executePress({ keys: args.text as string, signal }).then((result) => {
              const keys = typeof args.text === "string" ? args.text.toLowerCase() : "";
          ...
          case "hold_key":
            return await executeHoldKey({
              keys: args.text as string,
              durationMs: ((args.duration as number) ?? 0.5) * 1000,
              signal,
            });
          case "read_clipboard":
            return await executeReadClipboard();
          case "write_clipboard":
            return await executeWriteClipboard({ text: args.text as string });
          case "open_app":
          case "switch_app":
  ...
  return {
    ...
    schema: ComputerToolSchema as unknown as Record<string, unknown>,
    execute,
  };
}
