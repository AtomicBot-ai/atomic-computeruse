...
export function createMacOsVisionOcrAdapter(): OcrAdapter {
  const visionScriptPath = resolveNativeScript("macos/vision-ocr.swift");
  ...
  return {
    async recognizeText(params: RecognizeTextParams): Promise<OcrResult | null> {
      ...
      const { stdout } = await execFileAsync(
        "xcrun",
        ["swift", visionScriptPath, params.imagePath],
        {
